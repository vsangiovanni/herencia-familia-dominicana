...
- Ajuste de cache PWA posterior: `public/sw.js` subio a cache `legado-sangiovanni-v7`, deja de responder assets desde cache y pasa las solicitudes por red con `cache: no-store`. `src/lib/registerServiceWorker.ts` fuerza `registration.update()` al cargar. Si Vite detecta un chunk viejo con `vite:preloadError`, `src/main.tsx` desregistra service workers, limpia caches y recarga con query de cache-bust. Esto evita que celulares/PWA mezclen `index.html` nuevo con chunks viejos y caigan en flash/pantalla blanca.
