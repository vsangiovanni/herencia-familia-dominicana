...
- Ajuste de cache PWA posterior: `public/sw.js` subio a cache `legado-sangiovanni-v6` y `src/lib/registerServiceWorker.ts` fuerza `registration.update()` al cargar. Esto evita que celulares/PWA sigan sirviendo bundles viejos cuando se corrigen velocidad de typewriter, fotos de Maria Rosa Grisolia o musica.
